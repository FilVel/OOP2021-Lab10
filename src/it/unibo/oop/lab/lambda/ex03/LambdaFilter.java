...
public final class LambdaFilter extends JFrame {
    ...
    private static final String NO_STRING = "(\\s|\\p{Punct})+";

    private enum Command {
        IDENTITY("No modifications", Function.identity()),
        TO_LOWERCASE("Convert to lowercase", string -> string.toLowerCase()),
        COUNT_CHARS("Count the number of chars", string -> Integer.toString(string.length())),
        COUNT_LINES("Count the number of lines", string -> Long.toString(string.chars().
            filter(ch -> ch == '\n').count() + 1)),
        ALPHABET_ORDER("List all words in alphabetical order", string -> 
            Arrays.stream(string.split(NO_STRING))
                .sorted().collect(Collectors.joining("\n"))),
        COUNT_WORD("Show how many times a word has been used", string ->
            Arrays.stream(string.split(NO_STRING))
